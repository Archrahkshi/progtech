...
def fill_from(database):
    ...
    return [
        Marriage(
            groom_name=' '.join(line[:3]),
            groom_birthday=line[3],
            bride_name=' '.join(line[4:7]),
            bride_birthday=line[7],
            marriage_date=line[8],
            registry_number=line[9]
        ) for line in map(lambda line: line.split(), database.readlines())
    ]
...
for size in SIZES:
    ...
    for groom_name, marriage in zip(
            map(lambda obj: obj.groom_name, marriages_unsorted[size]),
            marriages_unsorted[size]
    ):
        multimaps[size][groom_name].append(marriage)
